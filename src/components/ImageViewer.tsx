...
const ImageViewer = (props: IProps) => {
  ...
  const handleClick = useCallback(
    (event: any) => {
      ...
      const checkClass = event.target.classList.contains('viewer__slide')
  ...
  return (
    <div
      ...
      className={`${styles.wrapper} viewer__modal`}
      onKeyDown={handleKeyDown}
      onClick={handleClick}
      style={props.backgroundStyle}
    >
      <span
        className={`${styles.close} viewer__close`}
      ...
      {props.src.length > 1 && (
        <span
          className={`${styles.navigation} ${styles.prev} viewer__previous`}
      ...
      {props.src.length > 1 && (
        <span
          className={`${styles.navigation} ${styles.next} viewer__next`}
          onClick={() => changeImage(1)}
        >
      ...
      <div
        className={`${styles.content} viewer__modal-content`}
        onClick={handleClick}
      >
        <div className={`${styles.slide} viewer__slide`}>
